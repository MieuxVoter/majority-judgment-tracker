...
def votes_to_merit_profiles(votes: List[List[Grade]], grades: List[Grade]) -> List[Dict[Grade, int]]:
    """
    ...
    votes: List[List[Grade]]
        Vote of each participant, each subList contains the grade for each candidate
    grades: List[Grade]
        List of grade of the vote, ex: [1,2,3], meaning voters can only attribute 1, 2 or 3.

    Returns
    -------
    A list of dictionaries which contains the number of votes for each grade
    """

    assert len(votes) > 0, "Empty list of votes"

    num_candidates: int = len(votes[0])
    profiles: List[Dict[Grade, int]] = [dict.fromkeys(grades, 0) for _ in range(num_candidates)]
...
def sort_by_gauge_with_index(gauges: List[MajorityGauge]) -> List[Tuple[int, MajorityGauge]]:
    by_gauge: List[Tuple[int, MajorityGauge]] = sorted(enumerate(gauges), key=lambda x: getattr(x[1], "gauge"))
    by_sign: List[Tuple[int, MajorityGauge]] = sorted(by_gauge, key=lambda x: getattr(x[1], "sign"))
    return sorted(by_sign, key=lambda x: getattr(x[1], "grade"))
...
class MajorityValue:
    ...
    def __post_init__(self):
        if self.values == []:
            votes = [i for grade, num in self.profile.items() for i in [grade] * num]
...
def sort_by_value_with_index(values: List[MajorityValue]) -> List[Tuple[int, MajorityGauge]]:
    return sorted(enumerate(values), key=lambda x: getattr(x[1], "values"), reverse=True)
